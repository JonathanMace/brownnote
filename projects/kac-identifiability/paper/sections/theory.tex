...
nonlinearly.  Expanding around the spherical limit $\varepsilon = 0$:
\begin{enumerate}
  ...
  \begin{equation}\label{eq:Delta_Ink}
    \Delta I_n^{(k)}
    = k\,a^{2}\!\int_0^1 P_n^2(\eta)\,
      \frac{\eta^2 - \tfrac{1}{2}}{a^{2k+2}}\,\mathrm{d}\eta.
  \end{equation}
  The key observation is that
  $\int_0^1 P_n^2(\eta)\,(\eta^2 - \tfrac{1}{2})\,\mathrm{d}\eta$ depends on
...
correction enters at order $\varepsilon^2$, the condition number diverges as
$\varepsilon \to 0$ according to a power law:
...
remaining parameters.  Section~\ref{sec:results} confirms this scaling
numerically and determines the exponent $\alpha$.
