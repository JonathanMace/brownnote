...
pelvic floor musculature.  By Rayleigh's theorem, imposing a partial rigid
boundary on a vibrating structure raises its natural frequencies, since the
constraint eliminates admissible trial functions and increases the Rayleigh
quotient.  The free-shell model therefore provides a \emph{lower} bound on
the resonant frequencies; the true $f_2$ with pelvic constraint would be
higher still.  This effect is not captured in the present analysis and
represents the most significant geometrical limitation of the model.
