...
$c/a \approx 0.67$~\cite{Mace2025browntone}---the bladder is approximately
...
satisfies the condition comfortably.  The Love--Kirchhoff hypothesis
(normals remain straight and normal to the deformed mid-surface) is
...
log-linear interpolation between vibrometry-derived
bounds~\cite{Nenadic2013,Barnes2016}:
...
\end{equation}
where $E_\mathrm{min} = \SI{10}{\kilo\pascal}$ at $V_\mathrm{min} =
\SI{50}{\milli\litre}$ and $E_\mathrm{max} = \SI{200}{\kilo\pascal}$ at
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lcccccc}
    ...
    Elastic modulus & $E$ & 10.0 & 19.5 & 52.8 & 200.0 & kPa \\
...
abdominal model~\cite{Mace2025browntone}, the restoring force is dominated
...
Feit~\cite{junger2012sound}, the natural frequency of flexural mode $n$ is
...
$f_2 = \SI{13.4}{\hertz}$, $f_3 = \SI{23.5}{\hertz}$, and
$f_4 = \SI{34.8}{\hertz}$.  The $n = 1$ mode corresponds to rigid-body
...
At $f_2 \approx \SI{13}{\hertz}$ the acoustic wavelength in air is
$\lambda \approx \SI{26}{\metre}$, so the dimensionless wavenumber is
\begin{equation}\label{eq:ka}
  ka = \frac{2\pi f_2 R}{c_\mathrm{air}} \approx 0.010 \ll 1.
\end{equation}
...
\begin{equation}\label{eq:peff2}
  ...
  \approx p_\mathrm{inc} \times 1.0 \times 10^{-4}.
\end{equation}
...
\end{equation}
At $f_2 = \SI{13.4}{\hertz}$, $T \approx 0.31$: the pelvic system
...
\begin{equation}\label{eq:R_coupling}
  \mathcal{R} = \frac{T(f_2) \times H(f_2)}{(ka)^2} \approx \frac{0.31 \times 2.5}{1.0 \times 10^{-4}} \approx 7{,}600.
\end{equation}
Mechanical vibration transmitted through the pelvic skeleton is approximately
four orders of magnitude more effective at exciting bladder wall modes than
airborne sound.
