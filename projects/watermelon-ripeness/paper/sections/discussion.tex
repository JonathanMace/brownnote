...
\begin{itemize}
  \item \textbf{Equivalent-sphere approximation.}  The present model
    ...
    introduces negligible error in the predicted tap-tone frequency,
    since the dominant physics is controlled by the shell's average
    radius rather than its local curvature.  A Ritz variational model
    with explicit $\zeta$-dependent basis functions (e.g.\ Legendre
    polynomials in oblate spheroidal coordinates) would be needed to
    capture genuine shape effects and is identified as future work.
...
a cultivar-independent calibration constant that maps the
result onto a ripeness scale without requiring detailed material
characterisation.
