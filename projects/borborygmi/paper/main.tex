...
\affiliation[github]{organization={GitHub, Inc.}, city={San Francisco, CA}, country={United States}}
...
\SIrange{1}{50}{\milli\litre}, spanning the clinically observed
...
with five distinct mechanisms producing frequencies that span the clinically
observed spectrum.  The model requires no free parameters: all inputs are
measurable anatomical and material properties---gas pocket volume, intestinal
wall stiffness, lumen diameter, and constriction geometry.  The
tissue-constrained bubble model, which augments the Minnaert formula with
elastic wall stiffness and inertia, reproduces the healthy bowel sound band
of \SIrange{200}{550}{\hertz} for pocket volumes of
\SIrange{1}{50}{\milli\litre}.  A mode-transition map reveals which resonance
...
Gas sound speed & $c_g$ & 343 & \si{\metre\per\second} \\
...
forcing.  For the canonical parameters, the constrained bubble mode is
dominant for volumes above approximately \SI{0.22}{\milli\litre}, with a
crossover to the axial piston mode at smaller volumes (where the slug length
becomes very short and the axial spring stiffness rises).
...
The constrained bubble model spans \SIrange{135}{440}{\hertz} over this
volume range, covering the majority of the clinically observed healthy bowel
...
\fbox{\parbox{0.9\textwidth}{\centering\vspace{3cm}
  \textbf{Figure placeholder}: All five resonance modes vs.\ gas pocket volume.
  Grey shading indicates the clinical borborygmi band
  (\SIrange{200}{550}{\hertz}).
  \vspace{3cm}}}
...
the dominant (lowest-frequency) mode changes.  For the canonical parameters,
a single crossover occurs at $V \approx \SI{0.22}{\milli\litre}$
($f \approx \SI{667}{\hertz}$), where the axial piston mode (dominant at
very small volumes) yields to the constrained bubble mode.

Above this crossover, the constrained bubble mode remains dominant across
the entire physiologically relevant range (\SIrange{1}{50}{\milli\litre}),
confirming it as the primary mechanism for healthy bowel sounds.  The
Helmholtz mode consistently lies above both bubble modes, becoming relevant
only when narrow constrictions are present.

\begin{figure}[t]
\centering
\fbox{\parbox{0.9\textwidth}{\centering\vspace{3cm}
  \textbf{Figure placeholder}: Mode transition map on log--log axes.
  Background shading indicates the dominant mode at each volume.
  \vspace{3cm}}}
\caption{Mode transition map showing the five resonance mechanisms across
  ...
  The constrained bubble mode dominates above
  $V \approx \SI{0.22}{\milli\litre}$.}
...
proportionally.  At \SI{10}{\milli\litre}, the constrained frequency shifts
from approximately \SI{215}{\hertz} at $E = \SI{1}{\kilo\pascal}$ to
\SI{310}{\hertz} at $E = \SI{100}{\kilo\pascal}$---a clinically significant
shift that provides a potential mechanism for pathological spectral changes
in conditions involving bowel wall fibrosis or inflammation.

\paragraph{Tube diameter.}
The constrained bubble frequency is weakly sensitive to tube diameter, since
the equivalent spherical radius $R$ depends on volume alone.  However, tube
...
The mode transition map (Fig.~\ref{fig:transition_map}) reveals a simple
landscape: the constrained bubble mode dominates across virtually the
entire physiologically relevant volume range, with only a narrow crossover
to the axial mode below \SI{0.22}{\milli\litre}.  This simplicity is itself
a useful result, as it indicates that a single-mode model (the constrained
bubble) is sufficient for most clinical interpretations.
...
require a more detailed geometrical treatment.
...
\begin{enumerate}
  \item The tissue-constrained bubble model, which augments the classical
    ...
    frequencies of \SIrange{135}{440}{\hertz} for gas pocket volumes of
    \SIrange{1}{50}{\milli\litre}.  This range spans the clinically observed
    healthy bowel sound band of \SIrange{200}{550}{\hertz} without
    curve-fitting or free parameters.

  \item A mode-transition map across the volume--frequency space identifies
    the constrained bubble mode as dominant for pocket volumes above
    $\sim$\SI{0.22}{\milli\litre}, covering the entire physiologically
    relevant range.  The Helmholtz and axial piston modes provide secondary
    contributions that account for high-pitched obstruction sounds and
    sub-audible rumbling, respectively.
  ...
  \item Wall stiffness is a secondary but clinically significant parameter:
    a tenfold increase (from \SI{10}{\kilo\pascal} to
    \SI{100}{\kilo\pascal}, representing fibrotic bowel) raises the predicted
    frequency by approximately 40\%, suggesting that pathological changes in
    wall compliance could produce detectable acoustic signatures.
\end{enumerate}
