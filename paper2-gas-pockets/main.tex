...
from tens to hundreds of hertz, depending on geometry, but sub-resonantly amplify tissue displacement as
...
abdominal flexural modes and represents the most plausible acoustic
mechanism identified to date for airborne-infrasound-induced GI
...
Our companion study \citep{Mace2026browntone} demonstrated that
...
with the whole-cavity flexural mode ($n = 2$) pathway from \citet{Mace2026browntone}.  The
...
$\xi \approx \SI{0.014}{\um}$ (energy-consistent; \citealp{Mace2026browntone})---more than an
...
Here $L_\text{eff}$ represents the effective acoustic path length from
mouth to colon ($\sim$\SI{5}{\metre} of GI tract), and
$d = \SI{10}{\milli\metre}$ is a conservative lower bound for the
average intestinal lumen diameter (typical range
\SIrange{10}{25}{\milli\metre}).
Below $f_H$, the air column can quasi-statically equalize pressure,
partially short-circuiting the transduction mechanism; above $f_H$,
air-column inertia prevents equalization and the constrained-bubble model
...
\begin{enumerate}
  ...
  \item \textbf{Partial equalization reduces but does not eliminate
        ...
        ratio ${\sim}0.8$ for an open segment).  Quantitatively, however,
        the margin above the \piezo{} threshold is modest: even for the
        largest pocket considered (\SI{100}{\milli\litre}, spherical), the
        maximum survivable reduction factor is only ${\sim}2.7\times$
        before the displacement falls below \SI{0.5}{\um}.  A factor-of-three
        reduction in effective $\Delta P$ would therefore bring all pockets
        below the mechanotransduction threshold at \SI{120}{\decibel}.
        This underscores the importance of the sealed-pocket defence
        (items~1--2 above): for gas pockets isolated by fecal or liquid
        plugs---the physiologically typical configuration---equalization
        cannot occur and the constrained-bubble model applies in full.
\end{enumerate}
...
flexural resonance and is the most plausible acoustic mechanism
identified to date by which airborne infrasound at realistic exposure levels ($\leq$\SI{120}{\decibel}) could
