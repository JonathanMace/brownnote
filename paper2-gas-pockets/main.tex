...
% ====================================================================
\begin{abstract}
Exposure to high-intensity infrasound (\SIrange{1}{20}{\hertz}) has been
anecdotally associated with gastrointestinal (GI) discomfort, yet whole-body
acoustic models predict negligible tissue displacement at realistic sound
pressure levels (SPL).  We propose that \emph{intraluminal gas pockets} in
the GI tract act as impedance-matching acoustic transducers, converting
airborne pressure oscillations to local wall strain far more efficiently than
whole-cavity resonance.  Using a constrained-bubble model---Minnaert
dynamics modified for the elastic intestinal wall and cylindrical lumen
geometry---we show that gas pockets with volumes of
\SIrange{5}{100}{\milli\litre} have resonant frequencies in the hundreds of
hertz, but sub-resonantly amplify tissue displacement by acting as compliant
inclusions in an otherwise incompressible medium.  The SPL threshold for
activating \piezo{} mechanosensitive ion channels (\SI{0.5}{\um}
displacement) ranges from \SI{111}{\decibel} for a \SI{100}{\milli\litre}
pocket to \SI{120}{\decibel} for a \SI{5}{\milli\litre}
pocket---a~\SI{9}{\decibel} range that maps directly to inter-individual
variability in bowel gas content.  A Monte Carlo population model
($N = 10\,000$) incorporating physiological gas-volume distributions shows
that 100\% of the simulated population exceeds the \piezo{} activation
threshold at \SI{120}{\decibel}.  This gas-pocket transduction pathway is
7--10$\times$ more efficient than airborne coupling to abdominal flexural
modes and represents the only plausible mechanism by which airborne
infrasound could induce GI mechanotransduction.  The model predicts enhanced
susceptibility in individuals with elevated bowel gas (post-prandial states,
irritable bowel syndrome, aerophagia), consistent with the sporadic and
individually variable nature of reported effects.
\end{abstract}
...
$\sim$\SI{2900}{\hertz}, far above the infrasound range.  The relevant
...
$\rho_f$   & Tissue/fluid density & \SI{1050}{\kilo\gram\per\metre\cubed} & \citet{Fung1993} \\
...
resonance frequencies range from $\sim$\SI{150}{\hertz} (large spherical
pockets) to $>\SI{1000}{\hertz}$ (small cylindrical pockets)---well above
the infrasound range.  This is a critical distinction from earlier analyses
that sought resonance \emph{at} infrasound frequencies.
...
representative gas pocket sizes.  All frequencies exceed \SI{100}{\hertz},
confirming that gas pockets do not \emph{resonate} at infrasound
frequencies.  However, as we show below, the sub-resonant compliance
mechanism operates efficiently across the entire infrasound band.
...
\midrule
  1  & Spherical    &  6.2 & 524  & 1014 \\
  5  & Spherical    & 10.6 & 307  &  610 \\
 10  & Spherical    & 13.4 & 244  &  492 \\
 20  & Spherical    & 16.8 & 193  &  399 \\
 50  & Spherical    & 22.9 & 142  &  299 \\
100  & Spherical    & 28.8 & 113  &  243 \\
 10  & Cylindrical  & 15.0 &  --- &  443 \\
 50  & Cylindrical  & 15.0 &  --- &  443 \\
100  & Cylindrical  & 15.0 &  --- &  443 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
  ...
  \item For \textbf{cylindrical} pockets, the threshold is nearly
        independent of volume ($\sim$\SI{118}{\decibel}), because the
...
is that \textbf{100\% of the population exceeds the \piezo{} activation
threshold} at this SPL, because even the smallest gas pocket
($\sim$\SI{3}{\milli\litre}) in the least-gassy individual produces
sufficient wall displacement.
...
\textbf{7--10$\times$ larger} than the whole-cavity pathway at the same
SPL, across the entire range from \SI{80}{\decibel} to \SI{150}{\decibel}.

At \SI{120}{\decibel}, the whole-cavity $n = 2$ mode yields
$\xi \sim \SI{e-4}{\um}$---four orders of magnitude below the \piezo{}
threshold.  In contrast, a \SI{100}{\milli\litre} gas pocket yields
$\xi \sim \SI{1.8}{\um}$, comfortably above threshold.
...
constrained-bubble pathway is 7--10$\times$ more efficient than whole-cavity
