...
As of late March 2026, all five papers have completed at least one full internal review cycle under the three-reviewer panel protocol (Reviewer~A: domain significance; Reviewer~B: adversarial rigour; Reviewer~C: computational reproducibility). The programme's computational suite comprises 203 passing tests, and the repository has accumulated 136+ merged pull requests across 20 specialised AI agents. Prof.\ Brian Mace (University of Auckland) is confirmed as active collaborator and co-author on all five manuscripts.

\subsection{Paper 1 --- Submission-Ready}

All three internal reviewers returned \textbf{ACCEPT}. A submission-readiness audit identified minor tone issues in the abstract and acknowledgements, which are being addressed. The paper is ready for upload to the JSV editorial portal once final polish is complete.

\subsection{Paper 2 --- Minor Revision (Post-Fix)}

A fatal equation flaw was discovered during the latest review round: the constrained-bubble formulation did not recover the Minnaert free-bubble limit when tissue constraint was removed. The fix corrected the \SI{10}{\mL} pocket frequency from \SI{1916}{\Hz} to \SI{222}{\Hz} and restored physical consistency. Four Minnaert-recovery regression tests were added to the suite (now 203 tests total). Post-fix Reviewer~C assessment: \textbf{MINOR REVISION}, with residual stale prose from the rewrite being corrected.

\subsection{Paper 3 --- Minor Revision}

First review (Reviewer~A): \textbf{MINOR REVISION}. The allometric data sourcing requires greater transparency, and the quasi-universal $\Pi_0$ claim needs quantification of inter-species variability (coefficient of variation $\approx 4.5\%$). All issues are tractable.

\subsection{Paper 4 --- Minor Revision (Post-Fix)}

Post-fix Reviewer~C assessment: \textbf{MINOR REVISION}. Three corrections were applied: the $k_\mathrm{shell}$ exponent in the stiffness formulation, the displacement reference level, and the strain ratio calculation. Twenty-seven bladder-specific tests are passing.

\subsection{Paper 5 --- Major Revision (Post-Fix)}

Post-fix Reviewer~B assessment: \textbf{MAJOR REVISION}. The radial breathing mode equation was found to be dimensionally incorrect, and the axial mode volume threshold was wrong by an order of magnitude. The core constrained-bubble physics is confirmed sound; corrections to the secondary mechanisms are underway.
...
The research campaign spanned \textbf{52.1 hours} of wall-clock time and involved \textbf{123 human prompts}, \textbf{246 subagent launches}, and approximately \textbf{316 million tokens} of language-model processing. The read-to-write ratio across the system was 1.15:1 --- the agents spent nearly as much time inspecting and reviewing their own output as they did generating it. This is not an artefact; it was designed in.
