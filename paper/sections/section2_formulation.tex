...
\begin{table}[htbp]
  ...
  \begin{tabular}{lcccl}
    ...
    Elastic modulus & $E$ & 0.1 & MPa & Composite wall \cite{Deeken2017} \\
    Poisson's ratio & $\nu$ & 0.45 & --- & FEM convention \cite{Spadoni2024} \\
    ...
    Intra-abdominal pressure & $P_\mathrm{iap}$ & 1000 & Pa & Resting IAP \cite{Novak2021} \\
    Loss tangent & $\eta$ & 0.25 & --- & Soft-tissue viscoelasticity \cite{Sack2023} \\
    \hline
  \end{tabular}
\end{table}
...
$E = \SI{0.1}{\mega\pascal}$ lies within the range reported for composite
abdominal wall tissue at physiological strains \cite{Deeken2017};
$\nu = 0.45$ matches the nearly incompressible convention adopted in recent
abdominal-wall finite-element modelling \cite{Spadoni2024}; resting
intra-abdominal pressures of 5--12\,mmHg correspond to approximately
\SIrange{0.67}{1.60}{\kilo\pascal}, placing
$P_\mathrm{iap} = \SI{1.0}{\kilo\pascal}$ near the physiological centre
\cite{Novak2021}; and modern elastography reviews place the soft-tissue loss
tangent broadly in the 0.1--0.4 range, making $\eta = 0.25$ a conservative
mid-range value \cite{Sack2023}.  Published values of $\eta$ for abdominal
...
$n$-th order component of the pressure field scales as \cite{Junger1986}:
...
\begin{equation}\label{eq:coupling_ratio_def}
  \mathcal{R} = \frac{\xi_\mathrm{mech}}{\xi_\mathrm{air}} \sim 10^4 \text{--} 10^5.
\end{equation}
