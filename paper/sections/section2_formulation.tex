...
in fluid-filled shell analysis \cite{junger2012sound,lamb1882}, an
...
$h = 10$\,mm and $R_\mathrm{eq} \approx 160$\,mm, $h/R \approx 0.06$.  The
Love--Kirchhoff hypothesis (normals remain straight and normal to the
deformed mid-surface) is therefore applicable.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lcccl}
    ...
    Elastic modulus & $E$ & 0.1 & MPa & Relaxed passive tissue \\
    Poisson's ratio & $\nu$ & 0.45 & --- & Nearly incompressible \\
    Wall density & $\rho_w$ & 1050 & kg/m$^3$ & Soft tissue \\
    Fluid density & $\rho_f$ & 1040 & kg/m$^3$ & Water-like \\
    Fluid bulk modulus & $K_f$ & 2.2 & GPa & Water at 37$^\circ$C \\
    Intra-abdominal pressure & $P_\mathrm{iap}$ & 1000 & Pa & Resting supine \\
    Loss tangent & $\eta$ & 0.30 & --- & Viscoelastic tissue \\
    \hline
  \end{tabular}
\end{table}


\subsection{Shell Constitutive Model}\label{sec:constitutive}

The abdominal wall is modelled as a Kelvin--Voigt viscoelastic material with
complex modulus
...
\begin{equation}\label{eq:D}
  D = \frac{Eh^3}{12(1 - \nu^2)}.
\end{equation}
Published values of $\eta$ for abdominal soft tissue range from 0.15 to 0.40
at low frequencies \cite{parker2011imaging}, corresponding to
$Q = 2.5$--$6.7$.  The baseline value $\eta = 0.30$ ($Q = 3.3$) represents a
central estimate.


\subsection{Breathing Mode ($n = 0$)}\label{sec:breathing}
...
\approx 2.4 \times 10^5$\,Pa/m) by five orders of magnitude.  \emph{The
...
\begin{equation}\label{eq:f0}
  ...
  {\rho_w h + \rho_f R}} \approx 2900\;\text{Hz},
\end{equation}
...
\subsection{Flexural Modes ($n \geq 2$)}\label{sec:flexural}
...
\cite{junger2012sound}, the flexural mode of order $n$ has three contributions
to the restoring force per unit area:
...
For the baseline parameters (Table~\ref{tab:params}), $f_2 = 4.4$\,Hz,
$f_3 = 6.7$\,Hz, and $f_4 = 9.4$\,Hz.  These lie squarely in the 4--10\,Hz
...
acoustic field.  At 7\,Hz, the acoustic wavelength in air is $\lambda \approx
49$\,m, so the dimensionless wavenumber is
\begin{equation}\label{eq:ka}
  ka = \frac{2\pi f R}{c_\mathrm{air}} \approx 0.017 \ll 1.
\end{equation}
...
$n$-th order component of the pressure field scales as \cite{junger2012sound}:
...
\begin{equation}\label{eq:peff2}
  ...
  \approx p_\mathrm{inc} \times 2.8 \times 10^{-4}.
\end{equation}
...
At 120\,dB SPL ($p_\mathrm{inc} = 20$\,Pa), equation~\eqref{eq:xi_air}
gives $\xi_\mathrm{air} \approx 0.14$\,$\mu$m for $n = 2$ --- well below the
0.5--2.0\,$\mu$m activation threshold of mechanosensitive ion channels
\cite{coste2010piezo1}.
...
\begin{equation}
  \xi_\mathrm{mech} \approx 7{,}000\;\mu\text{m} = 7\;\text{mm},
\end{equation}
...
conditions provides a dimensionless measure of the coupling disparity:
\begin{equation}\label{eq:coupling_ratio}
  \mathcal{R} = \frac{\xi_\mathrm{mech}}{\xi_\mathrm{air}} \sim 10^3 \text{--} 10^4.
\end{equation}
...
because both pathways excite the same modal structure.  The coupling ratio is
the central result of this work: it provides a quantitative explanation for why
...
infrasound at comparable frequencies is not.
