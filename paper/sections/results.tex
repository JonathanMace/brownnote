...
\begin{table}[htbp]
  ...
  Airborne displacement $\xi_\mathrm{air}$ is the pressure-based upper bound;
  the energy-consistent value is ${\sim}13\times$ smaller.}
...
Table~\ref{tab:bc_results} presents the results.  The free-sphere Ritz
frequency ($f_1 = \SI{4.13}{\hertz}$) reproduces the analytical solution
exactly, validating the numerical framework.  The most constrained case---25\%
of the shell surface clamped---reduces $f_1$ by only \SI{11.4}{\percent}.  Even
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lcc}
    \toprule
    Configuration & $f_1$ (Hz) & Shift from free \\
    \midrule
