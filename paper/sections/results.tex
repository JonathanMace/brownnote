...
A small number of Monte Carlo samples ($<0.3\%$) produced nonphysical
parameter combinations and were excluded from the summary statistics.
...
\begin{table}[htbp]
  ...
  Airborne displacement $\xi_\mathrm{air}$ and the ratio $\mathcal{R}$ use the
  pressure-based airborne upper bound at \SI{120}{\decibel}; the
  energy-consistent airborne displacement is ${\sim}13\times$ smaller, so the
  corresponding ratio at fixed mechanical loading is ${\sim}13\times$ larger.}
  ...
  \begin{tabular}{lccc}
    ...
    \hline
    $f_2$ & 6.4 & [3.3, 15.5] & Hz \\
    $\xi_\mathrm{air}$ (120 dB) & 0.18 & [0.11, 0.39] & $\mu$m \\
    $\xi_\mathrm{mech}$ (0.5 m/s$^2$) & 1800 & [300, 8200] & $\mu$m \\
    $\mathcal{R}$ (pressure-based airborne) & $10^4$ & [$2 \times 10^3$, $3 \times 10^4$] & --- \\
    SPL for PIEZO (pressure-based, 1 $\mu$m threshold)\footnote{The pressure-based displacement
      overestimates the true wall motion by a factor of ${\approx}\,13$
      because it neglects the extremely low radiation efficiency
      ($\zeta_\mathrm{rad}/\zeta_\mathrm{struct} \sim 10^{-14}$).
      Using the energy-consistent displacement
      (\S\ref{sec:coupling}), the PIEZO threshold rises to
      ${\approx}\,\SI{158}{\decibel}$---a \SI{23}{\decibel} correction
      that represents the physically realisable SPL.}
      & 135 & [128, 139] & dB \\
    SPL for PIEZO (energy) & ${\approx}\,158$ & --- & dB \\
    \hline
  \end{tabular}
\end{table}
...
loading, its $\mathcal{R}$ row should not be confused with the headline
energy-consistent SDOF upper bound of $6.6 \times 10^4$ quoted in
...
Table~\ref{tab:bc_results} presents the results.  The boundary-condition study
uses a slightly different parameter set ($R = \SI{0.162}{\metre}$,
$\rho_f = \SI{1000}{\kilo\gram\per\metre\cubed}$,
$P_\mathrm{iap} = \SI{1500}{\pascal}$) to match the mesh geometry; however, the
relative frequency shifts between boundary conditions are the parameter of
interest.  The free-sphere Ritz frequency ($f_2 = \SI{4.13}{\hertz}$) agrees
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lcc}
    ...
    \midrule
    Free sphere (Ritz) & 4.13 & 0\% (validates analytical) \\
...
added mass swamps the boundary stiffening effect.  The \SI{6}{\percent} shift
...
sphere model \emph{overestimates} flexural frequencies by 4--17\%
...
\SI{4.0}{\hertz} to approximately \SI{3.8}{\hertz}---still within the
