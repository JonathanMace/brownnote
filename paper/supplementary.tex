...
\begin{align*}
  k_{\mathrm{shell}} &= \frac{2 \times 10^{5} \times 0.01}{0.157^{2}\times 0.7975}
    \approx \SI{1.0e5}{\pascal\per\metre}, \\[4pt]
...
of $\sim\!4\times10^{5}$; hence $K_{f}$ completely dominates the breathing
mode stiffness.
...
(main text, Section~4) overestimates the true value by $\sim$40\%
because it does not enforce energy conservation.  The self-consistent
derivation, following the reciprocity formulation of Junger \& Feit (2012),
proceeds as follows.
...
\end{align*}
The pressure-based estimate is approximately $12.9\times$ larger because it
does not account for the extremely low radiation efficiency
($\zeta_{\mathrm{rad}}/\zeta_{\mathrm{struct}} \sim 10^{-14}$).  Both values
are far below the PIEZO1/2 mechanotransduction threshold
...
\begin{itemize}[nosep]
  ...
  \item Dominant factor: $E$, followed by geometry ($a$, $c/a$) and $P_{\mathrm{iap}}$.
\end{itemize}
...
\paragraph{Findings.}
The sphere model overestimates flexural frequencies by 11--20\%.  The error
...
within 4\% of the homogeneous model at $E = \SI{0.1}{\mega\pascal}$.
...
\begin{equation}
  ...
  \tag{S21}
\end{equation}
at the inner wall.  The associated viscous damping ratio addition is
(Landau \& Lifshitz, \S24)
\begin{equation}
...
via FEA validation results (\texttt{fea\_modal\_results.json}).
...
\paragraph{Key result.}
The maximum frequency shift from boundary conditions is 11\%.  This
...
\midrule
Sphere $\to$ oblate spheroid    & 11--20\%      & Curvature distribution \\
Homogeneous $\to$ multilayer    & $<$4\%        & Muscle tone \\
...
\texttt{mechanotransduction.py} &
  PIEZO1/2 activation thresholds and SPL required for channel opening. \\
