...
$\sigma_1\geq\sigma_2\geq\sigma_3\geq 0$.  The conditioning metric is
\[
...
\begin{enumerate}
  ...
  $\varepsilon=\sqrt{1-c^2/a^2}$ and examine the near-spherical limit
  $\varepsilon\to 0$ together with the canonical oblate point.
  ...
  $\mathrm{SO}(3)$ symmetry behind.
\end{enumerate}

For the oblate family, the smallest singular value is well represented near the
spherical limit by
\begin{equation}
  \sigma_{\min}(\varepsilon)
  =
  \sigma_0 + \lambda_1 \varepsilon^2 + O(\varepsilon^4),
  \label{eq:sigma-min-expansion}
\end{equation}
where $\sigma_0>0$ is the fitted intercept and $\lambda_1$ is the leading
curvature coefficient \cite{BrowntoneP8}.  The associated crossover parameter
is
\[
  \varepsilon_c=\sqrt{\sigma_0/\lambda_1}=\num{1.48}.
\]
Because $\varepsilon_c>1$, the fitted intercept remains influential throughout
the physical oblate range considered here.
