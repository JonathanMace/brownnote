% ====================================================================
%  Paper 5 — What Pitch Is a Growling Stomach?
%  Working draft — standard article class (JASA initial submission)
% ====================================================================
...
\author{%
  ...
  \small \textsuperscript{1}Microsoft Research, Redmond, WA, United States\\
  \small \textsuperscript{2}Acoustics Research Centre, Department of Mechanical and
         Mechatronics Engineering, University of Auckland, Auckland, New Zealand\\[2pt]
  \small \textsuperscript{*}Corresponding author: jonathanmace@microsoft.com%
}
...
comprising four resonance mechanisms: (i)~tissue-constrained bubble
resonance incorporating intestinal wall elasticity (which reduces to the
classical Minnaert frequency in the limit of vanishing wall stiffness),
(ii)~Helmholtz resonance through peristaltic constrictions,
(iii)~axial piston modes of cylindrical gas slugs, and (iv)~radial breathing
...
\noindent\textbf{Keywords:} borborygmi, bowel sounds, Minnaert resonance, bubble acoustics,
gastrointestinal acoustics, Helmholtz resonator
...
with four principal mechanisms (the classical Minnaert frequency arising as a
limiting case of the constrained bubble model) producing frequencies that
overlap the clinically observed spectrum.  The model requires no fitted parameters: all inputs are
...
thickness $h$, and density $\rho_w$.  Four primary resonance mechanisms are
considered, each corresponding to a distinct physical geometry and oscillation
pattern; the classical Minnaert frequency is recovered as a limiting case of
the constrained bubble model (Sec.~\ref{sec:constrained}).  Table~\ref{tab:params} summarises the canonical parameter values
used throughout.
...
together.  The per-displacement stiffness (force per unit wall area per unit
radial displacement) for the cylindrical geometry is
\begin{equation}
  k_\mathrm{gas} = \frac{2\gamma P_0}{R_l}, \qquad
  k_\mathrm{wall} = \frac{Eh}{R_l^2(1-\nu^2)},
  \label{eq:radial_stiffness}
\end{equation}
where the factor 2 (rather than 3 for the sphere) reflects cylindrical
geometry, and the division by $R_l$ (or $R_l^2$ for the wall term) ensures
dimensional consistency with the spherical model
(Eq.~\eqref{eq:stiffness}).  The effective mass per unit area is
\begin{equation}
...
\begin{equation}
  f_r = \frac{1}{2\pi}\sqrt{\frac{k_\mathrm{gas} + k_\mathrm{wall}}
    {m_\mathrm{fluid} + m_\mathrm{wall}}}.
  \label{eq:radial}
\end{equation}

For a \SI{30}{\milli\metre} tube, $f_r = \SI{163}{\hertz}$.  This frequency
is largely independent of gas pocket volume (depending only on tube
properties) and lies below the clinically reported bowel sound range of
\SIrange{200}{550}{\hertz}.  Like the axial piston mode, the radial breathing
mode contributes to the low-frequency rumbling component of gut sounds rather
than the characteristic mid-frequency borborygmi.
...
forcing.  For the canonical parameters, the radial breathing mode
(\SI{163}{\hertz}, independent of pocket volume) is the lowest-frequency mode
at small volumes; above approximately \SI{3.5}{\milli\litre}, the axial
piston mode takes over because its frequency drops with increasing slug
length ($f_\mathrm{ax} \propto V^{-1/2}$, compared with
$f_c \propto V^{-1/3}$ for the constrained bubble).

Both the radial and axial modes lie below the clinically reported bowel sound
band for most physiological volumes: $f_\mathrm{ax} < \SI{100}{\hertz}$ for
$V > \SI{10}{\milli\litre}$, and $f_r = \SI{163}{\hertz}$ throughout.
Consequently, the constrained bubble mode dominates the \emph{clinically
audible} band (\SIrange{200}{550}{\hertz}) across most of the
physiologically relevant volume range.  However, for small gas pockets
($V < \SI{2.5}{\milli\litre}$), the axial mode remains above
\SI{200}{\hertz} and competes with the constrained bubble mode within the
clinical band---for example, at $V = \SI{1}{\milli\litre}$,
$f_\mathrm{ax} = \SI{314}{\hertz}$ vs.\ $f_c = \SI{440}{\hertz}$.
...
modes with a \SI{10}{\milli\litre} gas pocket in a
...
breathing mode at \SI{163}{\hertz} also lies below the clinical band.
...
\midrule
Free Minnaert       & Eq.~\eqref{eq:minnaert}    & 244  & Limiting case of constrained \\
...
Radial breathing    & Eq.~\eqref{eq:radial}       & 163  & Below clinical band \\
\bottomrule
\end{tabular}
\end{table}
...
Figure~\ref{fig:modes_vs_volume} shows all mode frequencies as a
...
\caption{Predicted borborygmi frequencies as a function of gas pocket volume
  for all resonance mechanisms.  Shaded region: clinically observed
...
mode frequencies on logarithmic axes across a volume range of
...
the radial breathing mode (\SI{163}{\hertz}) is the lowest-frequency mode
below approximately \SI{3.5}{\milli\litre}, above which the axial piston
mode takes over as its frequency decreases with increasing slug length.
...
(\SIrange{5}{50}{\milli\litre}).  However, $f_\mathrm{ax}$ is sub-audible
at larger volumes ($< \SI{100}{\hertz}$ for $V > \SI{10}{\milli\litre}$),
and the radial mode sits at a fixed \SI{163}{\hertz} below the clinical band.
The constrained bubble mode is therefore the dominant mechanism within the
\emph{clinically audible} band, confirming it as the primary contributor to
auscultated bowel sounds.  For small pockets ($V < \SI{2.5}{\milli\litre}$),
the axial mode enters the clinical frequency range and contributes alongside
the constrained bubble mode.  The Helmholtz mode consistently lies above both
...
\caption{Mode transition map showing all resonance mechanisms across
  ...
  radial breathing mode (\SI{163}{\hertz}) and axial piston mode have the
  lowest overall frequencies but are predominantly sub-audible at
...
diameter strongly influences the axial and radial modes.  For the canonical
\SI{30}{\milli\metre} lumen, the radial breathing mode sits at
\SI{163}{\hertz}, below the clinical band; for larger-diameter segments
(colon, $d \approx \SIrange{40}{60}{\milli\metre}$), it decreases further
into the sub-audible regime.
...
Among the resonance mechanisms considered, the tissue-constrained bubble
...
is not the default geometry.  The axial and radial modes both operate below the clinical
frequency band and require elongated or tube-filling gas column geometries.
...
landscape.  By the lowest-frequency criterion, the radial breathing mode
dominates below $V \approx \SI{3.5}{\milli\litre}$ and the axial piston mode
dominates above this volume, because its frequency drops more steeply with
volume ($f_\mathrm{ax} \propto V^{-1/2}$ vs.\ $f_c \propto V^{-1/3}$ for
the constrained bubble).  However, both modes are predominantly sub-audible:
$f_r = \SI{163}{\hertz}$ (constant), and $f_\mathrm{ax} < \SI{100}{\hertz}$
for $V > \SI{10}{\milli\litre}$.  Within the audible band
(\SIrange{200}{550}{\hertz}), the constrained bubble mode is the dominant
contributor for volumes above $\sim$\SI{2.5}{\milli\litre}.  Below this
volume, the axial mode also falls within the clinical band and may contribute
to the multi-tonal character of small-pocket borborygmi.  This distinction
is clinically important: a single-mode model based on the constrained
bubble is sufficient for interpreting most auscultatory findings, even though
the formal lowest-frequency modes are sub-audible.
...
differently at different frequencies.  At the constrained bubble frequency
of \SI{223}{\hertz}, $Q = 4$ implies a free ring-down time of approximately
\SI{18}{\milli\second}, which is 30--45$\times$ shorter than the observed
\SIrange{0.5}{0.8}{\second} bowel sound burst durations~\cite{Ching2012}.
This discrepancy indicates that clinically observed borborygmi are sustained
excitation events---driven by ongoing peristaltic forcing over hundreds of
milliseconds---rather than free ring-downs of a briefly excited resonator.
The quality factor $Q = 4$ thus characterises the spectral width of each
resonance, not the temporal duration of the sound event.
...
frequencies of intestinal gas pockets, comprising four acoustic
mechanisms---tissue-constrained bubble resonance (with the classical Minnaert
frequency as a limiting case), Helmholtz resonance, axial piston modes, and
radial breathing modes.  The principal findings are:
...
\begin{enumerate}
  ...
  \item A mode-transition map across the volume--frequency space shows that
    the radial breathing mode (\SI{163}{\hertz}) and the axial piston mode
    have the lowest overall frequencies, but both are predominantly
    sub-audible ($f_\mathrm{ax} < \SI{100}{\hertz}$ for $V >
    \SI{10}{\milli\litre}$).  The constrained bubble mode therefore dominates
    the clinically audible band across most of the physiologically relevant
    volume range, though the axial mode competes within the clinical band
    for small pockets ($V < \SI{2.5}{\milli\litre}$).  The
