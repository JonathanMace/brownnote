...
\begin{table}[t]
  ...
  \caption{Canonical parameters for the Crimson Sweet watermelon at
    optimal ripeness.  Geometry ($a$, $c$, $h$) and densities are
    representative values drawn from Sadrnia et
    al.~\cite{Sadrnia2008} and De Belie et al.~\cite{DeBelieetal2000}.
    The rind modulus is an assumed mid-range value for ripe fruit based
    ...
    water-like media.  The loss tangent is representative of ripe
    rind tissue.}
  \label{tab:canonical}
  \begin{tabular}{llccc}
    \toprule
    ...
    Rind modulus (ripe) & $E_{\mathrm{rind}}$ & 50   & MPa & Assumed$^b$ \\
    Poisson's ratio    & $\nu$               & 0.40  & --- & Assumed$^c$ \\
    ...
    \multicolumn{5}{l}{\footnotesize $^b$\,Assumed mid-range value
      for ripe fruit from reported literature ranges.} \\
