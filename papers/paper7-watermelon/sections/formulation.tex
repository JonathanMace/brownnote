...
\begin{table}[t]
  \centering
  \caption{Canonical parameters for the Crimson Sweet watermelon at the
    reference stiffness state used throughout this paper.  Geometry ($a$, $c$, $h$) and densities are
    representative values drawn from Sadrnia et
    al.~\cite{Sadrnia2008} and De Belie et al.~\cite{DeBelieetal2000}.
    The rind modulus is an assumed mid-range value for a representative
    market-ready fruit based
    on the ranges reported in those sources.  Poisson's ratio and turgor
    pressure are assumed from the literature range for soft biological
    tissues.  The flesh bulk modulus is taken as a standard value for
    water-like media.  The loss tangent is representative of moderately softened
    rind tissue.}
  \label{tab:canonical}
  \begin{tabular}{llccc}
    \toprule
    ...
    \midrule
    Semi-major axis    & $a$                 & 0.158 & m & Lit.$^a$ \\
    Semi-minor axis    & $c$                 & 0.123 & m & Lit.$^a$ \\
    Aspect ratio       & $\zeta = c/a$       & 0.78  & --- & Derived \\
    Equivalent radius  & $R_{\mathrm{eq}}$   & 0.145 & m & Derived \\
    Rind thickness     & $h$                 & 15    & mm & Lit.$^a$ \\
    Rind modulus (reference) & $E_{\mathrm{rind}}$ & 50   & MPa & Assumed$^b$ \\
    Poisson's ratio    & $\nu$               & 0.40  & --- & Assumed$^c$ \\
    Rind density       & $\rho_w$            & 1050  & kg/m$^3$ & Lit.$^a$ \\
    Flesh density      & $\rho_f$            & 950   & kg/m$^3$ & Lit.$^a$ \\
    Flesh bulk modulus  & $K_f$              & 2.2   & GPa & Standard$^d$ \\
    Turgor pressure     & $P_{\mathrm{int}}$ & 500   & Pa & Assumed$^c$ \\
    Loss tangent        & $\eta$             & 0.15  & --- & Assumed$^c$ \\
    \bottomrule
    \multicolumn{5}{l}{\footnotesize $^a$\,Measured from literature
      \cite{Sadrnia2008, DeBelieetal2000}.} \\
    \multicolumn{5}{l}{\footnotesize $^b$\,Assumed mid-range value
      for a representative market-ready fruit from reported literature ranges.} \\
    \multicolumn{5}{l}{\footnotesize $^c$\,Assumed from literature
      range for soft biological tissues.} \\
    \multicolumn{5}{l}{\footnotesize $^d$\,Standard value for
      water-like media.}
  \end{tabular}
\end{table}
...
dimensionless group (\S\ref{sec:Pi_geom}), we instead use
...
(Eqs.~\ref{eq:Pi_geom}--\ref{eq:Pi_geom_universal}) refer to this
volumetric form.
...
accelerometer signal).  For watermelons in the reference stiffness range, this
...
For the canonical reference watermelon ($n = 2$, $h = \SI{15}{\milli\metre}$,
...
E_{\mathrm{true}}$ is less than \SI{0.01}{\percent} for all constructed
stiffness scenarios tested (\S\ref{sec:forward}).
...
% =============================================================================
% 2.4  Dimensional analysis and the geometric invariant
% =============================================================================
...
group
\begin{equation}
  \Pi_{\mathrm{geom}} = f_2 \, R_{\mathrm{eq}}
...
Consequently, $\Pi_{\mathrm{geom}}$ is \emph{not} invariant across
constructed softening scenarios in which both~$E$ and~$h$ change simultaneously
(as may occur during biological ripening;
Table~\ref{tab:forward_results}).
Rather, $\Pi_{\mathrm{geom}}$ is a
...
The practical utility of $\Pi_{\mathrm{geom}}$ lies in
...
$E_{\mathrm{rind}}$ via rearrangement of Eq.~\eqref{eq:Pi_geom},
...
onto a common constant), not across biological quality states---the latter would
require that the geometry remain fixed during softening, which is only
...
$\Pi_{\mathrm{geom}} = 0.062 \pm 0.002$ (CoV~$= \SI{3.3}{\percent}$;
see Table~\ref{tab:Pi_geom} in \S\ref{sec:universal_curve}).  This
