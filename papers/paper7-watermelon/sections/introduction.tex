...
deep, resonant thump is often taken to signal good eating quality; a
dull thud suggests a firmer, less developed fruit; a hollow ring warns
that the flesh may have turned mealy.  This `thump test'' is remarkably
consistent across cultures, yet it has never been given a quantitative
physical model that connects the tap-tone frequency to the measurable
...
\emph{effective rind stiffness parameter}, not a validated biological
quality classification.  The chain from tap-tone frequency to rind modulus is
established here; the subsequent chain from modulus to biological
quality state requires cultivar-specific calibration against
...
cultivar-specific quality calibration.
...
inverse problem: \emph{Can one hear the effective rind stiffness of a
watermelon?}  We show that the answer is conditionally yes: the dominant
flexural mode $n = 2$ can be inverted to yield the rind modulus, and
that modulus is expected to vary with postharvest softening through
pectin degradation---although the mapping to biological ripeness has not
been experimentally demonstrated here.
...
\begin{enumerate}
  ...
  \item as a secondary result, introduce a dimensionless geometric
    invariant $\Pi_{\mathrm{geom}}$ that collapses cultivar-dependent
