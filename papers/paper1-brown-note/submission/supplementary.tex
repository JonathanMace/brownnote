...
(main text, Section~4) is approximately $6.5\times$ larger than the
...
\begin{equation}
  \sigma_{\mathrm{abs}}^{(n)} = \sigma_{\max}^{(n)}
    \times\frac{4\,\zeta_{\mathrm{rad}}\,\zeta_{\mathrm{struct}}}{(\zeta_{\mathrm{rad}} + \zeta_{\mathrm{struct}})^2} \,.
  \tag{S14}
  \label{eq:sigma_abs}
\end{equation}
...
\begin{align*}
  \xi_{\mathrm{energy}} &\approx \SI{0.028}{\micro\metre}, \\
  \xi_{\mathrm{pressure}} &\approx \SI{0.18}{\micro\metre}.
\end{align*}
The pressure-based estimate is approximately $6.5\times$ larger because it
does not account for the extremely low radiation efficiency
($4\zeta_{\mathrm{rad}}\zeta_{\mathrm{struct}}/\zeta_{\mathrm{total}}^2 \sim 3 \times 10^{-14}$).  Both values
...
SPL$_{\mathrm{PIEZO}}$ (pressure-based, 1\,\si{\micro\metre} threshold)\footnote{Pressure-based displacement
  estimate; the energy-consistent threshold is ${\approx}\,\SI{16}{\decibel}$
...
oblate spheroid was performed using
...
The real abdominal wall comprises six distinct layers.
...
$\mu_{\mathrm{cross}} \approx \SI{280}{\pascal\second}$, far above
physiologically realistic abdominal fluid viscosities.  The inviscid
...
\texttt{run\_convergence\_study.py} &
  Mesh and quadrature convergence study for the Rayleigh--Ritz solver. \\
\texttt{ritz\_convergence.py} &
  Ritz-basis convergence study for the $n=2$ flexural mode, showing monotone
  convergence from the 2-DOF approximation to the enriched 16-DOF result. \\
...
\end{verbatim}
    Output appears in \texttt{papers/paper1-brown-note/figures/}.
