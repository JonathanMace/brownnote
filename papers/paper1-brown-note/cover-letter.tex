...
Existence of the `Brown Note','' for consideration in the
\textit{Journal of Sound and Vibration}.
...
approximately $3.3 \times 10^4$ times stronger than airborne acoustic coupling at
\SI{120}{\decibel} SPL.  This four-order-of-magnitude disparity---rooted in
long-wavelength spatial filtering and the air--tissue impedance
mismatch---explains why WBV routinely
...
\begin{itemize}
  ...
  \item \textbf{Energy-consistent coupling analysis ($R \approx 3.3 \times 10^4$).}
    A reciprocity-based energy budget shows that airborne sound at
    \SI{120}{\decibel} SPL produces wall displacements of only
    \SI{0.028}{\micro\metre}---two orders of magnitude below known
    mechanotransduction thresholds (PIEZO1/2 channels).  Mechanical WBV at
    routine occupational levels produces displacements ${\sim}\,3.3 \times 10^4$ times
    larger.  The qualitative ordering of this coupling ratio persists across the
    full physiologically plausible parameter space.
  ...
  \item \textbf{Monte Carlo uncertainty quantification.}  A 10{,}000-sample
    ...
    that the coupling ratio spans $10^3$--$10^4$, with elastic modulus
...
journal's scope.  The $3.3 \times 10^4$ coupling disparity also has
