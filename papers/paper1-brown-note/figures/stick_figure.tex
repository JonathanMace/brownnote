...
\begin{scope}[shift={(-3.5,0)}]
  ...
  \node[right, font=\tiny] at (0.3,1.3) {$\xi = 0.014\,\mu$m};
  % Arms
  \draw (0,1.7) -- (-0.4,1.3);
  \draw (0,1.7) -- (0.4,1.3);
  % Legs
  ...
  \node[draw, rounded corners, fill=white, font=\tiny, align=center]
    at (0.8,2.8) {perfectly\\fine};

  \node[below, font=\small\bfseries, align=center] at (0,0) {(a) Airborne\\[1pt]``Nothing happens''};
\end{scope}
...
\begin{scope}[shift={(3.5,0)}]
  ...
  \node[below, font=\tiny] at (0,0.05) {$a = 0.5$\,m/s$^2$};
  ...
  \node[right, font=\tiny] at (0.35,1.3) {$\xi = 0.9$\,mm};
  ...
  \draw[->, decorate, decoration={snake, amplitude=1pt, segment length=3pt}]
    (0.15,0.7) -- (0.4,0.4);
  \node[right, font=\tiny, align=left] at (0.4,0.35) {bowel\\[-1pt]response};

  \node[below, font=\small\bfseries, align=center] at (0,0)
...
% Centre annotation
% =========================================================
\draw[<->, very thick] (-0.8,1.3) -- (0.8,1.3);
\node[above, font=\small, align=center] at (0,1.35) {$\times\, 66{,}000$};
