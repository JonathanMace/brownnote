...
\begin{table}[htbp]
  ...
  The airborne displacement and reference-pair displacement ratio
  $\mathcal{R}_\mathrm{full}$ use the energy-consistent formulation at
  \SI{120}{\decibel}; the pressure-based value is retained elsewhere only as a
  labelled upper bound for comparison.  The mechanical row uses the same
  reference loading as Section~\ref{sec:coupling}:
  $a_\mathrm{rms} = \SI{0.1}{\metre\per\second\squared}$.}
  ...
  \begin{tabular}{lccc}
    ...
    \hline
    $f_2$ & 6.4 & [3.3, 15.5] & Hz \\
    $\xi_\mathrm{air}$ (\SI{120}{\decibel}, energy-consistent) & 0.027 & [0.016, 0.059] & $\mu$m \\
    $\xi_\mathrm{mech}$ (\SI{0.1}{\metre\per\second\squared}) & 360 & [59, 1640] & $\mu$m \\
    $\mathcal{R}_\mathrm{full}$ (\SI{120}{\decibel} vs.\ \SI{0.1}{\metre\per\second\squared}) & $1.3 \times 10^4$ & [$2.4 \times 10^3$, $4.6 \times 10^4$] & --- \\
    SPL for PIEZO1/2 activation (energy-consistent, 1\,$\mu$m) & 152 & [145, 156] & dB \\
    \hline
  \end{tabular}
\end{table}
...
Table~\ref{tab:uq_summary} now uses the same energy-consistent airborne
displacement and the same \SI{120}{\decibel}/\SI{0.1}{\metre\per\second\squared}
reference pair as Section~\ref{sec:coupling}, so its
$\mathcal{R}_\mathrm{full}$ row is directly comparable with the canonical SDOF
upper bound of $3.3 \times 10^4$ quoted there.
...
absolute prediction.  To assess the adequacy of the two-degree-of-freedom
basis, we enriched it by adding higher-order Legendre polynomials up to
$N = 8$ per displacement component, giving a 16-degree-of-freedom generalised
eigenproblem.  Table~\ref{tab:ritz_convergence} and
Figure~\ref{fig:ritz_convergence} show that the canonical $n=2$ flexural
frequency converges monotonically from above, consistent with the Ritz
upper-bound property.  The single-term basis ($N=1$, 2~DOF) gives
$f_2 = \SI{3.7995}{\hertz}$, whereas the converged value is
\SI{3.6784}{\hertz}; the two-degree-of-freedom model therefore overestimates
the converged result by only \SI{3.3}{\percent}.  Effective convergence is
already reached at $N=3$ (6~DOF), so the reduced basis is adequate for the
boundary-condition comparison made here.  For the mesh-matched parameter set
used in Table~\ref{tab:bc_results}, the free-sphere Ritz frequency
($f_2 = \SI{4.13}{\hertz}$) agrees
...
\begin{table}[htbp]
  \centering
  \caption{Ritz basis convergence for the $n=2$ flexural mode.}
  \label{tab:ritz_convergence}
  \begin{tabular}{ccc}
    \toprule
    $N$ & DOFs & $f_2$ (Hz) \\
    \midrule
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\columnwidth]{figures/fig_ritz_convergence}
  \caption{Convergence of $f_2$ with Ritz basis enrichment.  The dashed line
  marks the converged value; the two-degree-of-freedom result ($N=1$) lies
  \SI{3.3}{\percent} above it.}
  \label{fig:ritz_convergence}
\end{figure}
...
Within this converged Ritz framework, the frequency appears relatively
insensitive to boundary conditions.  A plausible physical reason is
that the fluid added mass for the $n=2$ mode is $7.4\times$ the shell mass, so
the system inertia is dominated by the enclosed fluid rather than the wall.  In
a dry (empty) shell, the same boundary conditions would shift the frequency by
approximately \SI{33}{\percent}---but once the cavity is fluid-filled, the
added mass appears to swamp much of the boundary stiffening effect.
...
constraint, the net effect is a frequency decrease.  The illustrative
\SI{6}{\percent} shift
is well within the $\pm 50$--$100\%$ uncertainty band set by material property
variability (\S\ref{sec:results}), suggesting that the free-shell model is an
adequate first approximation for the present purposes.
