...
Love--Kirchhoff hypothesis \cite{Love1927} (normals remain straight and normal to the
deformed mid-surface) is therefore applicable.  Classical thin-shell theory is
used for analytical tractability; higher-order shear effects are expected to
perturb frequencies modestly rather than overturn the coupling argument.
This linear framework remains
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/fig_geometry_schematic.pdf}
...
order.  The $n = 1$ mode corresponds to rigid-body translation and therefore
has zero natural frequency for a free shell, so it is excluded from the
resonance discussion.  The $n = 2$ mode is consequently the lowest physical
flexural mode and the primary candidate for abdominal resonance; higher modes
($n = 3, 4, \ldots$) lie at progressively higher frequencies and are retained
for sensitivity studies, but the $n = 2$ family dominates the physiologically
relevant \SIrange{4}{8}{\hertz} band.  The fluid therefore does \emph{not} act
as a volumetric spring; it contributes only added mass.

For a partially constrained shell---as when the pelvis and spine anchor the
inferior portion---the $n = 1$ mode ceases to be a pure rigid-body translation
and becomes a support-dominated ``belly bounce'' with a frequency determined
primarily by the attachment stiffness rather than by shell elasticity.  Because
this frequency depends on the mechanical properties of the pelvic support
(which are not modelled here), the present analysis excludes it and focuses on
$n \geq 2$ deformational modes whose frequencies are governed by the
shell--fluid system itself.  We note, however, that the existence of a
constrained $n = 1$ mode would strengthen, not weaken, the case for mechanical
coupling: it would provide an additional low-frequency pathway for whole-body
vibration, while the airborne suppression at $ka \ll 1$ would remain equally
severe for $n = 1$ as for $n = 2$.

Following Lamb \cite{lamb1882} and Junger \& Feit
\cite{Junger1986}, with subsequent verification in modern pressurised-shell
...
$f_3 = 6.3$\,Hz, and $f_4 = 8.9$\,Hz (Figure~\ref{fig:modes}).  These lie
squarely in the 4--10\,Hz
range reported for abdominal resonance in ISO~2631 \cite{iso2631} and in the
experimental studies of Kitazaki \& Griffin \cite{Kitazaki1998} and
Mansfield \cite{mansfield2005human}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/fig_mode_shapes.pdf}
...
\end{equation}
The proportionality constant from the multipole expansion is
$(2n+1)/(2n+1)!!$ \cite{Junger1986}, which evaluates to $1/3$ for $n=2$.
In what follows, we set this prefactor to unity as a deliberate upper bound
on the modal pressure; the exact coefficient reduces the pressure-based
displacement estimate by a factor of three.
...
\begin{equation}\label{eq:peff2}
  p_\mathrm{eff}^{(2)} \propto p_\mathrm{inc} \, (ka)^2
  \sim p_\mathrm{inc} \times 1.3 \times 10^{-4},
\end{equation}
where the upper-bound convention (unity prefactor) has been adopted; the
exact multipole coefficient of $1/3$ would reduce this estimate by a factor
of three.
...
To place this displacement in biological context, we compare it with
cellular mechanotransduction benchmarks.  PIEZO1 and PIEZO2 are mechanically activated
ion channels that convert membrane deformation into cellular signals; reported
activation in cell-culture experiments begins for deformations of order
\SIrange{0.5}{2.0}{\micro\metre} \cite{coste2010piezo1}.  While these
cell-scale values cannot be directly translated to tissue-scale abdominal
wall displacements, they provide a useful order-of-magnitude reference.
At 120\,dB SPL ($p_\mathrm{inc} = 20$\,Pa), the energy-consistent reciprocity
analysis (derived in \S\ref{sec:energy_budget}) gives
an energy-consistent wall displacement of approximately
$0.028$\,$\mu$m for $n = 2$ --- well below the
\SIrange{0.5}{2.0}{\micro\metre} cellular activation benchmark.  (The pressure-based upper-bound
modal-displacement estimate from equation~\eqref{eq:xi_air} yields
approximately $0.18$\,$\mu$m, an overestimate
that violates the energy budget; see \S\ref{sec:energy_budget}.)
...
coupling, there is no $(ka)^n$ penalty: the impedance mismatch between bone
and peritoneal wall is orders of magnitude smaller than the air--tissue
contrast, and the vibration bypasses the geometric filtering entirely.  The
following analysis therefore represents an \emph{upper-bound} base-excitation
model in which the skeletal frame transmits vibration rigidly to the shell
boundary.
...
\end{equation}
which exceeds the cellular mechanotransduction benchmark by more than three orders
of magnitude.
...
conditions provides a dimensionless measure for any chosen exposure pair:
\begin{equation}\label{eq:coupling_ratio_def}
  \mathcal{R}_\mathrm{full} = \frac{\xi_\mathrm{mech}}{\xi_\mathrm{air}}.
\end{equation}

This ratio depends primarily on $ka$ and the excitation level, and
the qualitative ordering persists across plausible parameter ranges
because both pathways excite the same modal structure.  Section~\ref{sec:coupling}
shows that the underlying system property is the large separation between the
transfer functions $H_\mathrm{mech}$ and $H_\mathrm{air}$; the corresponding
dimensionless ratio then follows once a reference pressure and acceleration are
chosen.  For \SI{120}{\decibel} SPL airborne excitation and
$a_\mathrm{rms} = \SI{0.1}{\metre\per\second\squared}$ RMS whole-body
vibration, the resulting SDOF upper bound is
$\mathcal{R}_\mathrm{full} \approx 3.3 \times 10^4$ ($\Gamma_2 = 1$).  However,
vertical base excitation does not couple
perfectly into the $n = 2$ mode: the modal participation factor
$\Gamma_2 \approx 0.48$ (\S\ref{sec:coupling_ratio}) reduces the effective
mechanical displacement to about half the SDOF value, giving a corrected
ratio of $\sim 1.6 \times 10^4$.  Even with this correction, the four-order-of-magnitude
gap between pathways is unaffected.
