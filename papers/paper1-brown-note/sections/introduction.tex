...
neither penalty.  The resulting separation between the airborne and mechanical
transfer functions is the central system property we derive and the principal
...
\begin{enumerate}
  ...
  \item compared airborne and mechanical excitation pathways within a common
    energy-budget framework to obtain the governing transfer functions and the
    exposure-pair displacement ratios derived from them.
\end{enumerate}
...
The headline result is therefore not a single scenario-specific number but a
large separation between transfer functions.  At the canonical $n=2$
resonance, $H_\mathrm{mech}/H_\mathrm{air}
\approx 6.5 \times 10^6\,\si{\pascal\second\squared\per\metre}$ before the
modal-participation correction, and the corresponding reference-pair
displacement ratio for \SI{120}{\decibel} SPL airborne excitation and
$a_\mathrm{rms} = \SI{0.1}{\metre\per\second\squared}$ RMS whole-body
vibration is approximately $3.3 \times 10^4$ under the SDOF idealisation
($\Gamma_2 = 1$).  Including the canonical participation factor
$\Gamma_2 \approx 0.48$ reduces this to $\sim 1.6 \times 10^4$.  Monte Carlo
uncertainty quantification confirms that the transfer-function contrast
persists across physiologically realistic parameter ranges and resolves the
empirical asymmetry cleanly.  It explains why whole-body vibration at
\SIrange{4}{8}{\hertz} produces well-documented gastrointestinal effects, why
airborne infrasound at comparable frequencies does not, and why claims of a
brown note are not supported by the physics of airborne acoustic coupling.
