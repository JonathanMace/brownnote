...
\begin{enumerate}
  ...
  \item \textbf{Free-shell boundary conditions.}  The abdomen is partially
    ...
    ($f_2 = \SI{3.66}{\hertz}$, $-11.4\%$).  The basis-enrichment study in
    \S\ref{sec:bc_effects} shows monotone Ritz convergence from
    \SI{3.7995}{\hertz} to \SI{3.6784}{\hertz}, so the reduced two-degree-of-
    freedom basis carries only a \SI{3.3}{\percent} bias for the canonical
    $n=2$ mode.  These shifts therefore appear physically meaningful, while
    remaining illustrative because the constraint geometries are surrogate
    rather than anatomically exact.  The entire bracketed range lies well
    within the material-property uncertainty band.  Boundary
