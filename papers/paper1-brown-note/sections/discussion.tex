...
a fluid of density $\rho_f$ is \cite{minnaert1933}
\begin{equation}
  f_M = \frac{1}{2\pi a} \sqrt{\frac{3\gamma P_0}{\rho_f}},
\end{equation}
where $\gamma = 1.4$ is the ratio of specific heats (we use $\gamma = 1.4$
for a diatomic ideal gas; a more representative value for the typical
intestinal mixture of CO$_2$/CH$_4$/N$_2$ would be
$\gamma \approx 1.35$, introducing a ${\sim}\,2\%$ correction to the
Minnaert frequency) and $P_0$ is the ambient pressure.  For physiological pocket radii of
...
($T_I \approx 10^{-3}$), in the acoustically compact limit ($ka \ll 1$)
the body is too small to scatter the incident wave, and the interior
pressure equilibrates quasi-statically with the incident field to leading
order.  Pre-existing gas pockets
