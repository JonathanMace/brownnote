...
which is sensitive to tissue properties.
...
free-field intensity.  At steady-state resonance this power is entirely
dissipated by structural damping, so
\begin{equation}\label{eq:xi_energy}
  \xi_\mathrm{energy}
  = \sqrt{\frac{\sigma_\mathrm{abs}\,I}
               {2\,\zeta_\mathrm{struct}\,\omega_n^3\,M_\mathrm{eff}}},
\end{equation}
where $M_\mathrm{eff} = m_\mathrm{eff}^{(n)} \times 4\pi R^2$ is the total
modal mass.  The factor of $\zeta_\mathrm{rad}/\zeta_\mathrm{struct}
\sim 10^{-14}$ inside $\sigma_\mathrm{abs}$ is the physical origin of the
${\approx}\,13\times$ reduction from the pressure-based estimate
...
fact only the fraction $\zeta_\mathrm{rad}/\zeta_\mathrm{total}$ is absorbed.
