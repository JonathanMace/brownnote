...
and restores full rank.  In the intermediate eccentricity regime where the
shape-channel correction dominates the curvature-channel floor, the condition
number follows an approximate power law
$\kappa \approx C\,\varepsilon^{-\alpha}$, with
...
(Table~\ref{tab:power_law}).  As $\varepsilon \to 0$, the curvature channel
provides a finite floor, and $\kappa$ saturates at
$\kappa_\mathrm{floor}\approx 269$ rather than diverging.
...
For the Ritz model with fixed mode set $\mathcal{N}$, the smallest singular
value of the scaled Jacobian satisfies
\begin{equation}
  \sigma_{\min}\bigl(\mathbf{J}_s(\varepsilon;\mathcal{N})\bigr)
  =
  \sigma_0(\mathcal{N}) + \lambda_1(\mathcal{N})\,\varepsilon^{2}
  + O(\varepsilon^4)
  \qquad \text{as } \varepsilon \to 0^+,
\end{equation}
where $\sigma_0 > 0$ is the curvature-channel floor.  In the intermediate
eccentricity regime where $\lambda_1\varepsilon^2 \gg \sigma_0$, the
condition number follows an approximate power law
\begin{equation}
  \kappa\bigl(\mathbf{J}_s(\varepsilon;\mathcal{N})\bigr)
  \approx
  \frac{\sigma_{\max}}{\lambda_1(\mathcal{N})}\,\varepsilon^{-2}\bigl(1+o(1)\bigr).
\end{equation}
As $\varepsilon \to 0^+$, however, the condition number saturates at
$\kappa_\mathrm{floor} = \sigma_{\max}/\sigma_0 \approx 269$ (evaluated at
$\varepsilon = 0.01$, i.e.\ $\zeta = 0.99995$, with five modes) rather than
diverging.  The empirical exponents $\hat\alpha\in[2.16,\,2.66]$
(Table~\ref{tab:power_law}), fitted over the intermediate range, are
consistent with the theoretical $\alpha = 2$ to leading order; the
deviation from $2$ reflects the finite curvature-channel floor
rounding off the divergence.
\end{conjecture}
...
The numerics already suggest a clean intermediate-regime power law with
$\alpha\approx2$ and a finite curvature-channel floor, but a rigorous proof of
the $\sigma_{\min}$ expansion would turn a useful plot into an asymptotic
theorem --- and would give the floor, coefficient, and crossover eccentricity as
closed-form functions of the mode set.  That would be the crown jewel of the
present identifiability programme; at the time of writing, it remains work in
progress.
