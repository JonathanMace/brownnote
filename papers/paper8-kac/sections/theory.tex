...
The result leads to a quantitative framework for the condition number.  The
smallest singular value of the scaled Jacobian admits a regular perturbation
expansion~\cite{Kato1966,StewartSun1990}:
\begin{equation}\label{eq:sigma_min_expansion}
  ...
  + O(\varepsilon^6),
\end{equation}
where $\sigma_0 > 0$ is the \emph{curvature-channel floor} --- the residual
identifiability contributed by mode-dependent Legendre weights even at the
spherical limit --- and $\lambda_1\varepsilon^2$ is the \emph{shape-channel}
correction driven by eccentricity.  Since $\sigma_0 > 0$, the expansion is
\emph{regular}: the smallest singular value approaches a finite positive
limit as $\varepsilon \to 0$, and the condition number saturates at
\begin{equation}\label{eq:kappa_floor_theory}
  \kappa_\mathrm{floor}
  =
  \frac{\sigma_{\max}(0)}{\sigma_0}
\end{equation}
rather than diverging.  The crossover eccentricity
$\varepsilon_c = \sqrt{\sigma_0/\lambda_1}$ separates a curvature-dominated
regime ($\varepsilon < \varepsilon_c$, where the floor controls conditioning)
from a shape-dominated regime ($\varepsilon > \varepsilon_c$, where
eccentricity drives improvement).  Section~\ref{sec:results} determines
$\sigma_0$, $\lambda_1$, $\lambda_2$, and $\varepsilon_c$ numerically and
confirms that for the oblate Ritz model, $\varepsilon_c > 1$: the curvature
channel dominates across the entire physical eccentricity range.
