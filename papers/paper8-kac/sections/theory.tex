...
variances under the assumed noise model.  Note that since $\mathbf{F}$ is
defined using the scaled Jacobian $\mathbf{J}_s$, the diagonal elements of
$\mathbf{F}^{-1}$ yield lower bounds on the \emph{relative} (fractional)
parameter variances; the corresponding absolute bounds are obtained by
multiplying by $|\theta_j|^2$.
