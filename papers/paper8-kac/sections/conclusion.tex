...
\begin{enumerate}
  ...
  \item the condition number follows an approximate power law
  $\kappa\approx C\,\varepsilon^{-\alpha}$ in the intermediate eccentricity
  regime, with theoretical exponent $\alpha=2$ and empirical estimates
  $\hat\alpha\in[2.16,\,2.66]$; as $\varepsilon\to 0$ the curvature channel
  provides a finite floor at $\kappa_\mathrm{floor}\approx 269$;
...
The most interesting open mathematical target is the asymptotic expansion
\begin{equation}
  \sigma_{\min}(\varepsilon)
  = \sigma_0 + \lambda_1\,\varepsilon^{2} + O(\varepsilon^4)
  \qquad \text{as } \varepsilon \to 0^+,
\end{equation}
which implies the intermediate-regime power law
$\kappa \approx C\,\varepsilon^{-2}$ for $\lambda_1\varepsilon^2 \gg \sigma_0$
and a finite floor $\kappa_\mathrm{floor} = \sigma_{\max}/\sigma_0 \approx 269$
as $\varepsilon \to 0$.  This is currently supported numerically
(Table~\ref{tab:power_law}) and formalised in
Conjecture~\ref{conj:power_law}.  The fitted prefactor
$\hat C\approx24$--$26$ and the floor value provide
...
away from the spherical limit, and exactly where it saturates.  That would be
the crown jewel.  For the moment, the jewel remains in the workshop.
