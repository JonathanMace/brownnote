...
The companion papers established the ingredients that we now place in a common
mathematical frame.  Paper~1 distinguished the low-frequency flexural family
from the much higher breathing mode in a fluid-filled soft shell, and showed
that geometry controls not only resonance but also the efficiency with which
forcing projects onto those modes~\cite{BrowntoneP1}.  Paper~7 showed that an
equivalent-sphere model can nevertheless be operationally useful when the
inverse task is deliberately narrow and the geometry is known
independently~\cite{BrowntoneP7}.  Paper~8 then exposed the inverse price of
that reduction: once geometry and material must be inferred simultaneously, the
same equivalent-radius simplification becomes structurally
defective~\cite{BrowntoneP8}.  The present section upgrades that programme to
...
\end{equation}
Paper~8 reports $\kappa(\widetilde{J}(\boldsymbol{\theta}_*))=69.4$ for the
...
\end{equation}
with $\lambda_1>0$ for the canonical oblate family of Papers~8--10.  In
particular,
\begin{equation}
  \sigma_3(0)=0.
\end{equation}
...
\begin{lemma}[Forward adequacy does not imply inverse adequacy]
\label{lem:p10_forward_inverse}
...
evaluation in Paper~8 gives the associated numerical condition number
...
point, and Paper~8 gives the concrete value
$\kappa(\widetilde{J}_\mathrm{oblate})=69.4$ for the five-mode inverse
~\cite{BrowntoneP8}.  The two models are therefore close as forward predictors
yet radically different as inverse operators.  Paper~7 is the benign companion
example: there the equivalent-sphere watermelon model succeeds precisely because
geometry is fixed independently and the inverse target is effectively
one-dimensional~\cite{BrowntoneP7}.  Forward adequacy is not false; it is simply
insufficient for multivariate spectral inversion.
\end{proof}

Taken together, Theorem~\ref{thm:p10_scalar_reduction},
Theorem~\ref{thm:p10_lifting}, Proposition~\ref{prop:p10_asymptotics}, and
Lemma~\ref{lem:p10_forward_inverse} define the capstone thesis in mathematical
form.  Scalar geometric reductions collapse one inverse direction; oblate
curvature can restore it; the restoration vanishes quadratically at the exact
sphere; and forward success is not a licence for inverse use.  The results
