...
The programme developed thus far supplies three ingredients that are required in
...
present section upgrades that programme to
...
yet radically different as inverse operators.  An instructive companion
example is provided by the watermelon model in~\cite{BrowntoneP7}: there the
equivalent-sphere reduction succeeds precisely because geometry is fixed
independently and the inverse target is effectively one-dimensional.  Forward adequacy is not false; it is simply
...
sphere; and forward success does not justify inverse use.  The results
