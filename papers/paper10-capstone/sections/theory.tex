...
~\cite{BrowntoneP1,BrowntoneP8}.  The shell is modelled as free
(unconstrained); the resulting modal classification --- $n=1$ rigid-body
translation, $n\geq 2$ flexural modes, $n=0$ breathing mode --- relies on that
boundary condition.  We restrict attention to the flexural modes
...
amplified in the inferred parameters.  In practice, $\widetilde{J}$ is
evaluated by central finite differences with relative step $\delta=10^{-6}$.
...
--- is robust: the $(ka)^{2n+2}$ radiation-damping suppression reflects the
general multipole scaling and is not an artefact of spherical symmetry.  A quantitative
...
distinct meridional and circumferential curvature distributions.  In the Ritz
formulation detailed in Appendix~\ref{sec:appendix_ritz}, mode $n$ samples
those curvature fields through mode-specific weighting functions.
Schematically,
\begin{equation}
  f_n(\varepsilon)
  =
...
\begin{proposition}[Identifiability lifting by oblate asphericity]
\label{prop:p10_lifting}
...
\begin{proof}
The proof is by numerical construction at the canonical operating point,
extended by continuity.  For a fixed Ritz basis and a fixed measured-mode set,
the frequencies $f_n(\boldsymbol{\theta})$ are smooth functions of $(a,c,E)$
away from modal crossings, so the entries of
$\widetilde{J}(\boldsymbol{\theta})$ are smooth as well.  At the canonical
oblate operating point
\begin{equation}
  \boldsymbol{\theta}_*
  =
  \bigl(\SI{0.18}{\metre},\SI{0.12}{\metre},\SI{0.1}{\mega\pascal}\bigr),
\end{equation}
the numerical study in~\cite{BrowntoneP8} evaluates the scaled Jacobian by
central finite differences and reports
$\kappa(\widetilde{J}(\boldsymbol{\theta}_*))=69.4$ for the five-mode inverse
using modes $n=2,\ldots,6$.  A finite condition number requires all three
singular values to be strictly positive; therefore
$\sigma_3(\widetilde{J}(\boldsymbol{\theta}_*))>0$ and
...
on $\boldsymbol{\theta}$.  Since
$\sigma_3(\widetilde{J}(\boldsymbol{\theta}_*))$ is strictly positive, there
...
$U$ on which $\operatorname{rank}(\widetilde{J})=3$.
\end{proof}

The result is existential rather than global.  It does not say that every
...
intercept $\sigma_0>0$ was a numerical evaluation artefact: the
finite-difference Jacobian misbehaves near $\varepsilon=0$ because the oblate
spheroidal coordinate system becomes singular at the sphere, causing the
computed $\sigma_3$ to plateau at $O(10^{-2})$ rather than reaching the
theoretical zero.  This is not a deficiency of the Ritz model itself, whose
structure guarantees $\sigma_3(0)=0$ by symmetry, but rather a consequence of
evaluating derivatives numerically at a coordinate singularity.
What survives is a regular even expansion in the curvature parameter.
...
Moreover, $\operatorname{rank}(\widetilde{J}_0)=2$ exactly.  To see this, note
that for any Lamb-type spherical shell the flexural frequency scales as
$f_n \propto \sqrt{E}\,g_n(R/h,\nu,\ldots)$, so the scaled geometric sensitivity
$(\partial f_n/\partial R)(R/f_n)$ depends on $n$ through $g_n$ and the
material sensitivity $(\partial f_n/\partial E)(E/f_n)=1/2$ is mode-independent.
Two columns with different mode dependences are linearly independent,
confirming $\operatorname{rank}(\widetilde{J}_0)=2$.  (Numerically, the
finite-difference sphere-model Jacobian has $\widetilde{J}[:,a]/\widetilde{J}[:,c]=2.0$
to $10^{-9}$ precision, while the $R$-to-$E$ column ratio varies from $-3.0$ to
$-6.5$ across modes $n=2,\ldots,6$.)  Thus $\mu_3(0)=0$ is a simple eigenvalue of
$A_0=\widetilde{J}_0^\top\widetilde{J}_0$, which is the prerequisite for the
analytic perturbation expansion below.

To determine the leading power, consider the Gram matrix
$A(\varepsilon)=\widetilde{J}(\varepsilon)^\top\widetilde{J}(\varepsilon)$,
whose eigenvalues are $\mu_k=\sigma_k^2$.  The even expansion of
$\widetilde{J}$ gives
\begin{equation}
  A(\varepsilon)
  =
  A_0 + \varepsilon^2 A_2 + O(\varepsilon^4),
  \label{eq:p10_A_even_expansion}
\end{equation}
where
$A_0=\widetilde{J}_0^\top\widetilde{J}_0$ and
$A_2=\widetilde{J}_0^\top\widetilde{J}_2+\widetilde{J}_2^\top\widetilde{J}_0$.
Let $\mathbf{v}_0\in\ker(\widetilde{J}_0)$ be the unit null vector
(guaranteed to exist since $\sigma_3(0)=0$).  The first-order eigenvalue
perturbation of $\mu_3$ vanishes:
\begin{equation}
  \mathbf{v}_0^\top A_2\,\mathbf{v}_0
  =
  \mathbf{v}_0^\top\bigl(
    \widetilde{J}_0^\top\widetilde{J}_2
    +\widetilde{J}_2^\top\widetilde{J}_0
  \bigr)\mathbf{v}_0
  =
  0,
  \label{eq:p10_first_order_vanish}
\end{equation}
because $\widetilde{J}_0\,\mathbf{v}_0=\mathbf{0}$.  The eigenvalue
$\mu_3(\varepsilon)$ therefore enters at second order in $\varepsilon^2$,
i.e.\ at order $\varepsilon^4$.  By the analytic perturbation theory of
Kato~\cite{Kato1966}, the eigenvalue branches of $A(\varepsilon)$ are
themselves analytic in the perturbation parameter where they remain simple,
so $\mu_3(\varepsilon)=\lambda_1^2\varepsilon^4+O(\varepsilon^6)$ and hence
$\sigma_3(\varepsilon)=\lambda_1\varepsilon^2+O(\varepsilon^4)$.

For the canonical oblate family, Proposition~\ref{prop:p10_lifting} gives
$\sigma_3(\varepsilon)>0$ on an open set of $\varepsilon>0$, so the leading
coefficient satisfies $\lambda_1>0$, yielding
\eqref{eq:p10_sigma_asymptotic}.  The corrected near-spherical analysis
therefore removes the earlier fictitious intercept $\sigma_0>0$: the apparent
finite floor was a numerical evaluation artefact arising from finite-difference
Jacobian computation near the oblate spheroidal coordinate singularity at
$\varepsilon=0$, not a property of the Ritz model's algebraic structure.
\end{proof}

\begin{remark}[Independence from the near-spherical blend]
\label{rem:blend_independence}
Proposition~\ref{prop:p10_asymptotics} invokes Kato's analytic perturbation
theory, which requires the map $\varepsilon\mapsto\widetilde{J}(\varepsilon)$
to be real-analytic near $\varepsilon=0$.  The oblate Ritz energy functionals
are analytic in $a$ and~$c$ away from the coordinate singularity at $a=c$
(Appendix~\ref{sec:appendix_energy}), so this condition holds for the ideal
model family.  The code, however, introduces a $C^2$-continuous blend between
the oblate spheroidal fluid solution and the Lamb sphere formula for $c/a>0.98$.
This blend makes the numerical implementation $C^2$ but not analytic in the
transition region.  The key point is that the \emph{conclusions} of
Proposition~\ref{prop:p10_asymptotics} do not depend on the blend: the even
symmetry $\varepsilon\mapsto -\varepsilon$ and the rank-$2$ property of
$\widetilde{J}_0$ are structural features of the underlying analytic family.
The blend affects the numerically computed values of $\sigma_3$ in the
transition region (contributing to the plateau at $\sigma_3\approx 0.008$
visible in Figure~\ref{fig:condition_landscape}), but it does not alter
the algebraic structure that forces $\sigma_3(0)=0$ and
$\sigma_3\sim\varepsilon^2$.
\end{remark}
...
$n=2,\ldots,6$ is $\mathcal{E}_\mathrm{fwd}=0.0936<0.1$ (computed at the
canonical parameters of Table~\ref{tab:cross_application_evidence} via scaled-Jacobian
singular-value decomposition).  Thus the reduced
...
On the same parameter domain, Proposition~\ref{prop:p10_lifting} shows that the
...
Proposition~\ref{prop:p10_lifting}, Proposition~\ref{prop:p10_asymptotics}, and
