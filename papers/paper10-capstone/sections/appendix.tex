...
$d = a\varepsilon$.  In oblate spheroidal coordinates $(\xi,\eta,\varphi)$, the
shell surface sits at $\xi_0 = c/d$.  Here $\eta = \cos\theta$ ranges over
$[-1,1]$ with $\theta$ the polar angle, and $\varphi\in[0,2\pi)$ is the
azimuthal angle.  The metric function
\begin{equation}
  G(\eta) = a^2\eta^2 + c^2(1-\eta^2)
  \label{eq:app_metric}
\end{equation}
determines the position-dependent curvature.  The surface element is
$\mathrm{d}S = 2\pi\,a\sqrt{G}\,\mathrm{d}\eta$.

For each circumferential mode number $n\geq 2$, the trial displacement field
...
\begin{enumerate}
  ...
  \item \emph{Geometric prestress.}  The intra-abdominal pressure
    ...
    $N_1 = P_\mathrm{iap}\,R_2/2$, where $R_2(\eta) = a\sqrt{G}/c$ is the
...
\begin{equation}
  w(\eta) = \sum_{i=0}^{N-1}\beta_i\,P_{n+2i}(\eta), \qquad
...
convergence threshold.  For all practical purposes, the single-term Ritz basis
is sufficient for the spectral and inverse conclusions drawn in this paper.
