...
curvature appear on equal footing~\cite{Junger1986}.  Paper~1 adapted that
framework to the human abdomen and used a Rayleigh--Ritz approximation to
retain the features that matter for the present programme: curvature, wall
compliance, internal loading, and mode shape~\cite{BrowntoneP1}.  The
capstone does not repeat that derivation.  It uses the resulting structure as
...
\subsection{Companion papers: Act I --- the question}

\emph{Act~I} asked whether the notorious brown-note claim contained any
physically respectable core, and the answer turned out to be more interesting
than the folklore.  Paper~1 showed that the abdomen, modelled as a fluid-filled
oblate shell, does possess a genuine low-frequency flexural resonance, with the
second mode occurring at
\begin{equation}
f_{2} = \SI{3.95}{\hertz}.
...
airborne responses~\cite{BrowntoneP1}.  The resonance was real; the direct
acoustic route to it was not.

Paper~2 supplied the first important refinement~\cite{BrowntoneP2}.  Instead of
treating the abdominal cavity as the only relevant resonant structure, it
considered tissue-constrained gas pockets as local compliant inclusions capable
of much stronger pressure-to-motion transduction.  This did not contradict the
whole-cavity result of Paper~1.  It clarified it.  If one seeks a global
...
\subsection{Act II --- evidence across scales, organs, and pathways}

\emph{Act~II} tested whether the same shell-theoretic logic survived changes in
scale, organ, and excitation pathway.  Paper~3 recast the abdominal resonance
problem in dimensionless form and derived scaling laws that made the low-order
flexural behaviour comparable across species~\cite{BrowntoneP3}.  Its point was
not merely zoological.  By showing that the governing frequency scales can be
expressed through a compact set of geometric and mechanical groups, the paper
demonstrated that the infrasonic shell problem is structured by similarity,
rather than by the contingencies of one anatomical example.

Paper~4 transferred the framework to the urinary bladder and thereby tested it
on an organ whose geometry changes substantially with physiological
state~\cite{BrowntoneP4}.  The bladder calculations showed that fill-dependent
resonance cannot be interpreted as a simple size effect.  Increasing volume
...
Paper~5 shifted attention from infrasonic flexure to an audible internal
phenomenon: borborygmi~\cite{BrowntoneP5}.  Its constrained-bubble model
...
Paper~6 returned to low frequencies and extended the coupling question to the
sub-bass regime, where concert exposures and whole-body vibration are often
discussed together despite acting through rather different pathways
~\cite{BrowntoneP6}.  The conclusion was consistent with Paper~1.  Airborne
...
Act~II, the programme had assembled a coherent evidential base: across species,
...
\subsection{Act III --- from forward adequacy to inverse structure}

\emph{Act~III} turned from forward resonance prediction to the inverse question
of what one can infer from a measured spectrum.  Paper~7, on watermelon
tap-tone assessment, provided the constructive example~\cite{BrowntoneP7}.  It
showed that an equivalent-sphere shell model can be forward-adequate for a
...
measured resonance to an effective rind stiffness with useful fidelity.  The
watermelon paper therefore demonstrated that reduced geometry can be serviceable
when the application asks for classification or approximate property recovery,
rather than full geometric disentanglement.

Paper~8 exposed the inverse price of that simplification~\cite{BrowntoneP8}.
...
condition-number analysis showed that the equivalent-sphere model is
catastrophically ill-conditioned, whereas the full oblate-shell model restores
practical separation of parameter effects.  Just as importantly, Paper~8
...
Paper~9 condensed that observation into a short theorem-oriented note on
identifiability lifting~\cite{BrowntoneP9}.  Its contribution was to show that
...
because it rules out an easy but misleading slogan.  One cannot simply say that
